...
class SigTool:
    def __init__(self, apk_path, args):
        ...
        self.colon_upper = ColonUpperCase(
            apply_colons="-c" in args
            or "-uc" in args
            or "-fc" in args
            or "-fuc" in args,
            convert_uppercase="-u" in args
            or "-uc" in args
            or "-fu" in args
            or "-fuc" in args,
        )
                ...
                if b"-----BEGIN CERTIFICATE-----" in content:
                    ...
                    print(
                        self.formatter.format_error(
                            f"Error: The APK, RSA or PEM file at '{self.apk_path}' does not exist or is not accessible."
                        )
                    )
                    sys.exit(1)
    ...
    def extract_apk_info(self):
        try:
            ...
            return {
                "Error": self.formatter.format_error(f"Failed to extract APK info: {e}")
            }
    ...
    def extract_signature_hex(self):
        try:
            ...
            if self.signature_hex:
                self.signature_hex = self.colon_upper.convert_to_uppercase(
                    self.signature_hex
                )
        except Exception as e:
            print(self.formatter.format_error(f"Error: {e}"))
    ...
    def print_apk_info(self):
        if not os.path.isfile(self.apk_path):
            print(
                self.formatter.format_error(
                    f"Error: The APK file at '{self.apk_path}' does not exist or is not accessible."
                )
            )
            sys.exit(1)
    ...
    def print_default_results(self):
        ...
        if self.signature_hex is None:
            print(
                self.formatter.format_error(
                    "Error: Failed to extract signature hex from the APK file."
                )
            )
            sys.exit(1)

        hashes = self.calculate_hashes()
        crc32_hashcode = CRC32AndHashCodeCalculator(
            self.signature_hex
        ).calculate_crc32_and_hash_code()
        ...
        output += self.formatter.format_section(
            "CRC32 and hashCode Results", formatted_results
        )
    ...
    def print_encoded_results(self):
        ...
        output += self.formatter.format_result_two(
            "\nsignatures (Add '\\n' per 76 characters)",
            formatted_encoded_signature.lstrip("\n"),
        )
        ...
        self.add_section(
            "signatures (Add '\n' per 76 characters)",
            formatted_encoded_signature.replace("\\n", "\n"),
        )

        return output

    def print_pem_results(self):
        pem_certificate = PEMCertificateGenerator(
            self.signature_hex
        ).generate_certificate_details()
    ...
    def print_smali_results(self):
        smali_representation = SmaliByteArrayGenerator(
            self.signature_hex
        ).generate_smali()
    ...
    def run(self):
        try:
            ...
            if self.signature_hex is None:
                print(
                    self.formatter.format_error(
                        "Error: Signature hex is None, cannot proceed."
                    )
                )
                sys.exit(1)

            encoded_signature, encoded_hashes = self.encode_base64(
            ...
            if "-hmt" in self.args and file_type == "apk":
                ...
                output_directory = (
                    self.args[self.args.index("-o") + 1]
                    if "-o" in self.args
                    else os.path.dirname(os.path.abspath(self.apk_path))
                )

                if not os.path.isdir(output_directory):
                    print(
                        self.formatter.format_error(
                            f"Error: The specified path '{output_directory}' is not a valid directory."
                        )
                    )
                    print(
                        self.formatter.format_error(
                            f"Please provide a valid directory where the {output_filename} file can be saved."
                        )
                    )
                    sys.exit(1)
                ...
                print(
                    self.formatter.format_with_style(
...
def get_version():
    try:
        return importlib.metadata.version("sigtool")
...
"""
    ...
    parser.add_argument(
        "-c", action="store_true", help="Add colons to certificate hashes"
    )
    parser.add_argument(
        "-uc",
        action="store_true",
        help="Add colons to hashes and convert output to uppercase",
    )
    ...
    parser.add_argument(
        "-fc",
        action="store_true",
        help="Add colons to hashes and print all information",
    )
    parser.add_argument(
        "-fu",
        action="store_true",
        help="Convert output to uppercase and print all information",
    )
    parser.add_argument(
        "-fuc",
        action="store_true",
        help="Add colons to hashes, convert output to uppercase and print all information",
    )
    parser.add_argument(
        "-hmt",
        action="store_true",
        help="Generate and export hook of MT enhanced version",
    )
    parser.add_argument(
        "-o",
        type=str,
        help="Output results to a specified file path. If the path ends with '.json', results will be saved in JSON format.",
    )
    parser.add_argument(
        "-v",
        "--version",
