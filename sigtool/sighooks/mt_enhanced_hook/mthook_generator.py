...
class MTHookGenerator:
    def __init__(
        self, encoded_zip: str, apk_path: str, pkg_name: str, encoded_cert: str
    ):
    ...
    def add_lib_files(
        self, new_zip: zipfile.ZipFile, files: Dict[str, bytes], valid_libs: List[str]
    ) -> None:
        zip_lib_folder = "lib"
        for name, data in files.items():
    ...
    def add_assets_folder(
        self, new_zip: zipfile.ZipFile, files: Dict[str, bytes]
    ) -> None:
        for name, data in files.items():
    ...
    def add_dex_file(
        self, new_zip: zipfile.ZipFile, dex_output_path: str, new_dex_name: str
    ) -> None:
    ...
    def modify_zip(self, zip_data: bytes) -> bytes:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            return self.prepare_new_zip(
                files, dex_output_path, valid_libs, new_dex_name
            )
